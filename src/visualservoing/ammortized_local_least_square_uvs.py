...
class AmmortizedLocalLeastSquareUVS(Policy):
    ...
    def __init__(self, gain, min_experience=DEFAULT_CAPACITY,
        ...
        super(AmmortizedLocalLeastSquareUVS, self).__init__(gain= gain, state_extractor= state_extractor)
    ...
    def batch_knn(self, X_train, X_test, k = 50):
        #TODO:
        #this...is kinda bad idea for reeealy large matrices
        #at 100,000 for train set this will blow up your ram if you try to do all examples
        #
        [n, d] = X_train.shape
        [t, d] = X_test.shape
	
        term1 = torch.matmul(X_train**2, torch.ones(d,t)) 
        term2 = torch.matmul(torch.ones(n, d), X_test.transpose(0,1)**2) 
	
        term3 = 2 * torch.matmul(X_train, X_test.transpose(0,1))
        distances = term1 + term2 - term3

        distances[distances == 0.0] = np.inf
        top_k = torch.topk(distances, k = k, largest=False, sorted=True)[1]
        return top_k

    def build_knn_dataset(self):
        #calculate knn 
        #TODO: need to go and implement this...
        return 0.0
    ...
    def calculate_deltas_dX_dQ(self, S, Sp):
        #State  values
        ths = self.state_extractor.get_batch_angles(S)
        S_q = ths
        S_psn = self.state_extractor.get_batch_position(S)

        #State prime values
        ths = self.state_extractor.get_batch_angles(Sp)
        Sp_q = ths
        Sp_psn = self.state_extractor.get_batch_position(Sp)
        ...
        for e in range(epochs):
            ...
            for b in dataloader:
                ...
                if self.fit_inverse_relation:
                    ...
                    dX_fit = dX.unsqueeze(-1)
                    yhat_p = torch.matmul(iJ, dX_fit)
                    aux_loss = self.beta * loss_fn(yhat_p.squeeze(-1), dQ)
                    loss = loss + aux_loss
    ...
    def act(self, obs):
        ...
        if self._prev_obs is not None:
            #might need to copy
            self._memory.push(self._prev_obs, obs)

        self._prev_obs = obs
        return action

class ReverseAmmortizedLocalLeastSquareUVS(AmmortizedLocalLeastSquareUVS):
    def __init__(self, gain, min_experience=DEFAULT_CAPACITY,
                    capacity=DEFAULT_CAPACITY,
                    num_actuators=7, 
                    memory="fixed",
        ...
        super(ReverseAmmortizedLocalLeastSquareUVS, self).__init__(
                    gain = gain, 
